...
function gameLoop(time) {
  ...
  // ── Mission waypoint guidance ──
  const missionObjPos = missions.getCurrentObjectivePos();
  if (missionObjPos && !inVehicle && !inBank) {
    const dist = player.pos.distanceTo(missionObjPos);
    document.getElementById('status').textContent +=
      ` | OBJ: ${missions.getCurrentObjectiveDesc() || ''} (${dist.toFixed(0)}m)`;
  }
  if (racing.active) {
